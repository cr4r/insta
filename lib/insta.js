        ...
        if(image_url.substring(0,8) === 'https://' || image_url.substring(0,7) === 'http://'){
            ...
            request(image_url, (error, response, html) => {
                if(!error){
                    ...
                    if(response.statusCode === 200) { resolve({'status':'ok', 'result': [title, url, file, tagnya] }) };
            ...
            resolve({ 'status':'no', 'result': [{'message' : 'Invalid URL'}] });
        }
    }else{
        res.status(400).json({ 'status':'no', 'result': [{'message' : 'Provided invalid URL'}] });
    }
});